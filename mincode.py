...
Agent-agnostic dispatcher: wraps every script behind one `mincode <cmd>` CLI so
you never need to remember script names or the scripts/ path. Zero dependencies
...
Usage from anywhere:
    python /path/to/mincode-vuln-arch/mincode.py audit .
    # or after `pip install .` / copy to PATH:
    mincode audit .
...
# vault.py subcommands (append/verify/status/rotate-key/diff) keep their arg shape
VAULT_SUBCOMMANDS = {"append", "verify", "status", "rotate-key", "diff"}
...
def _run(script, argv):
    ...
    # ensure `import config` resolves inside scripts/
...
def main():
    ...
    # build subparsers that just forward everything
...
def _init(repo):
    ...
    if not os.path.exists(toml_dst):
        src = os.path.join(HERE, "mincode.toml.example")
